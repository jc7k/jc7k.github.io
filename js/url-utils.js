...
function getVideoPollingConfig() {
    ...
    return {
        videoId: urlParams.get('video') || 'f_N3PGvnVKg', // Default video
...
function getVideoId() {
    const urlParams = new URLSearchParams(window.location.search);
    return urlParams.get('video') || 'f_N3PGvnVKg';
}
...
function getValidatedVideoPollingConfig() {
    ...
    return {
        videoId: isValidVideoId(config.videoId) ? config.videoId : 'f_N3PGvnVKg',
